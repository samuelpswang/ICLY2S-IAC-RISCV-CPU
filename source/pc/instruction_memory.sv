...
initial begin
  $display("[DUT] Loading instr_data from f1_branch.hex file.");
  $readmemh("../program/f1/f1_branch.hex", instr_data);
end

always_comb begin
  if (A[2:0] == 2'b00) RD = {instr_data[A[ADDR_WIDTH-1:0]+3], instr_data[A[ADDR_WIDTH-1:0]+2], instr_data[A[ADDR_WIDTH-1:0]+1], instr_data[A[ADDR_WIDTH-1:0]]};
